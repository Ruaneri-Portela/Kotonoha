...
static std::string BuildString(const char *str,
  ...
  if (lastSlash == std::string::npos) {
    ...
    lastSlash += 1; // inicia após o último '/'
  }
    ...
    for (auto *event = classUp->eventsFromScript.data; event != nullptr;
      ...
      case PLAY_VOICE: {
        if (event->data.play_voice->path != nullptr &&
          ...
          gameplay->audio->AddMedia(BuildString(event->data.play_voice->path,
                                                assetsPath,
                                                useExtension ? ".OGG" : "")
                                        .c_str(),
                                    event->start, UINT64_MAX, false, "Voice");
          ...
          if (!lastCreateBg->empty() &&
            ...
            std::string path = BuildString(pathImg.c_str(), assetsPath,
                                           useExtension ? ".PNG" : "");
            SDL_IOStream *file = SDL_IOFromFile(path.c_str(), "rb");
            if (file != nullptr) {
              SDL_CloseIO(file);
              gameplay->image->Register(path.c_str(), event->start, event->end,
                                        1);
            }
          }
        }
      ...
      case PLAY_SE:
        if (event->data.play_se->path != nullptr &&
          ...
          gameplay->audio->AddMedia(BuildString(event->data.play_se->path,
                                                assetsPath,
                                                useExtension ? ".OGG" : "")
                                        .c_str(),
      ...
      case END_ROLL:
        if (event->data.path_end->path != nullptr &&
            SDL_strlen(event->data.path_end->path) > 0) {
          gameplay->video->Register(BuildString(event->data.path_end->path,
                                                assetsPath,
                                                useExtension ? ".WMV" : "")
                                        .c_str(),
      ...
      case PLAY_MOVIE:
        if (event->data.play_movie->path != nullptr &&
          ...
          gameplay->video->Register(BuildString(event->data.play_movie->path,
                                                assetsPath,
                                                useExtension ? ".WMV" : "")
                                        .c_str(),
      ...
      case CREATE_BG:
        if (event->data.create_bg->path != nullptr &&
          ...
          *lastCreateBg = event->data.create_bg->path;
          gameplay->image->Register(BuildString(event->data.create_bg->path,
                                                assetsPath,
                                                useExtension ? ".PNG" : "")
                                        .c_str(),
...
Event::Event(const char *orsPath, void *gameplay,
  ...
  for (auto *event = eventsFromScript.data; event != nullptr;
    ...
    case PRINT_TEXT: {
      ...
      ASS_Event *subtitleEvent =
          static_cast<Gameplay *>(gameplay)->sb->track->events;
      subtitleEvent +=
          static_cast<Gameplay *>(gameplay)->sb->track->n_events - 1;
      ...
      for (int i = 0;
           i < static_cast<Gameplay *>(gameplay)->sb->track->n_styles; i++) {
        ASS_Style *style =
            static_cast<Gameplay *>(gameplay)->sb->track->styles + i;
    ...
    case SetSELECT: {
      ...
      static_cast<Gameplay *>(gameplay)->prompt = new Prompt(
          options, &static_cast<Gameplay *>(gameplay)->promptId, event->start,
          event->end, static_cast<Gameplay *>(gameplay)->tm);
  ...
  if (parms[3] == nullptr || parms[4] == nullptr) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                 "Failed to allocate memory for EventManager");
