...
struct Kotonoha_audioDecode *Audio::AddMedia(const char *path, Uint64 start,
  ...
  std::vector<std::tuple<struct Kotonoha_audioDecode *, Sound::Channel::Pipe *>>
      *audioDecodesPipe = nullptr;
  ...
  if (audioDecodesPipe == nullptr) {
    ...
    audioDecodesPipe = &std::get<1>(
        mediaObjects.back()); // Obtém referência ao vetor recém-criado
  }

  // Inicializa e adiciona o novo Kotonoha_audioDecode ao vetor
  Kotonoha_audioDecode *newAudioDecode =
      Kotonoha_AudioInit(path, gettedCh->GetSpecs());
  ...
  Sound::Channel::Pipe *newPipe =
      gettedCh->AddPipe(RenderMedia, Kotonoha_AudioFree, newAudioDecode);
...
void Audio::RemoveMedia(struct Kotonoha_audioDecode *mediaPtr) {
  for (auto it = mediaObjects.begin(); it != mediaObjects.end();) {
    ...
    for (auto audioIt = audioDecodesPipe.begin();
         audioIt != audioDecodesPipe.end();) {
      ...
      if (mediaPtr == nullptr || std::get<0>(*audioIt) == mediaPtr) {
        ...
        audioIt =
            audioDecodesPipe.erase(audioIt); // Erase e continue a iteração
      } else {
        ++audioIt; // Avança apenas se nada foi removido
      }
    }

    // Se o vetor de audioDecodesPipe estiver vazio após a remoção, remove o
    // objeto de mediaObjects
