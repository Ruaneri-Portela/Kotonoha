...
Sound::Channel::Pipe::Pipe(KOTONOHA_AUDIO_COMPONENTS)
    : function(function), closeFucntion(closeFunction), parms(parms),
      data(nullptr), lenghtData(0), lastGetData(0) {
  Render();
}
...
void Sound::Channel::Render(void *userdata, SDL_AudioStream *astream,
  ...
  while (it != thisChannel->pipes.end()) {
    ...
    SDL_MixAudio(target, localData, thisChannel->spec.format, gettedSize,
                 thisChannel->volume * thisSound->volume);
    filled += gettedSize; // Atualiza a quantidade preenchida
    maxFilled = (maxFilled > filled) ? maxFilled
                                     : filled; // Atualiza o m�ximo preenchido
  ...
  SDL_PutAudioStreamDataNoCopy(astream, data, totalAmount, destroy, data);
}
