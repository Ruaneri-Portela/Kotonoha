...
Prompt::Prompt(std::vector<std::string> prompt, int *promptResult, Uint64 start,
               Uint64 end, Kotonoha_time *time)
    : prompt(std::move(prompt)), promptResult(promptResult), time(time),
...
enum Kotonoha_Scene_Status Prompt::Render(KOTONOHA_SCENE_CALL) {
  ...
  bool inRange;
  Sint64 diff;
  Kotonoha_timeGetFromEvent(prompt->time, prompt->startTime, prompt->endTime,
                            &inRange, &diff);
  if (!inRange)
  ...
  for (int i = 0; i < prompt->prompt.size(); i++) {
    ...
    TTF_GetStringSize(prompt->font, prompt->prompt[i].c_str(),
                      prompt->prompt[i].size(), &rect.w, &rect.h);
    ...
    SDL_Surface *surface =
        TTF_RenderText_Shaded(prompt->font, prompt->prompt[i].c_str(),
                              prompt->prompt[i].size(), color, bgColor);
...
Prompt::~Prompt() { TTF_CloseFont(font); }
