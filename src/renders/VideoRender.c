...
static bool renderVideo(struct Kotonoha_videoData *instance) {
  ...
  while (av_read_frame(instance->pFormatCtx, &packet) >= 0) {
    ...
    if (instance->frameTime == 0) {
      AVRational avFrameRate = av_guess_frame_rate(
          instance->pFormatCtx,
    ...
    instance->videoTime = Kotonoha_UtilsFFmpegGetTime(
        instance->pFormatCtx->streams[instance->videoStreamIndex],
        instance->pFrame->pts, instance->pFrame->pkt_dts);
    ...
    if (instance->pCodecCtx->hw_device_ctx) {
      ...
      if (!instance->pFrame) {
        SDL_LogError(SDL_LOG_CATEGORY_ERROR,
                     "Cannot allocate frame for HW data\n");
...
static bool initializeVideoContext(struct Kotonoha_videoData *instance,
  ...
  instance->swsCtx = Kotonoha_UtilsFFmpegCreateSwsContext(
      instance->pCodecCtx, instance->pFrame->format, instance->pCodecCtx->width,
...
struct Kotonoha_videoData *Kotonoha_VideoRenderInit(const char *filename,
  ...
  struct Kotonoha_videoData *videoInstance =
      (struct Kotonoha_videoData *)SDL_calloc(
          1, sizeof(struct Kotonoha_videoData));
  if (!videoInstance) {
    SDL_LogError(SDL_LOG_CATEGORY_ERROR,
...
enum Kotonoha_Scene_Status Kotonoha_VideoRenderProcess(void *userData,
  ...
  if (elapsedTime >= instance->frameTime) {
    ...
    AVFrame *pFrameRGB = Kotonoha_UtilsFFmpegSwsConvertFrame(
        instance->swsCtx, instance->pFrame, &buffer, instance->pFrame->height,
        instance->pFrame->width);

    if (pFrameRGB == NULL)
      return KOTONOHA_SCENE_FATAL_ERROR;

    SDL_UpdateTexture(instance->texture, NULL, pFrameRGB->data[0],
