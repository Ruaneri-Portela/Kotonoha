...
static char openBuf[4096];
static char appendBuf[4096];
...
static std::string Kotonoha_GetTemporaryScenePath(const std::string &path) {
  ...
  TempCleanupData cleanupData{std::string(prefPath), fileName};

  SDL_EnumerateDirectory(
      prefPath,
      [](void *userdata, const char *dirname,
        ...
        const std::string fullPath = data->directory + currentName;
        SDL_RemovePath(fullPath.c_str());
        return SDL_ENUM_CONTINUE;
      },
      &cleanupData);

  std::string tempPath = std::string(prefPath) + fileName + ".tmp.ks";
  SDL_free(prefPath);
  return tempPath;
}

static bool Kotonoha_LoadTextFile(const std::string &path,
                                  std::string &outText) {
  if (path.empty()) {
...
static bool Kotonoha_SaveTextFile(const std::string &path,
...
static bool Kotonoha_LoadSceneEditorFromDisk(Kotonoha::Gameplay *play) {
  ...
  if (!Kotonoha_LoadTextFile(play->scriptPath, sceneEditorText)) {
    return false;
  }
...
static bool Kotonoha_SaveSceneEditorToTemporary(Kotonoha::Gameplay *play,
...
static bool Kotonoha_OpenPaths(Kotonoha::Kotonoha *game, const char *buffer,
                               bool append) {
  bool ok = true;
...
static void Kotonoha_SelectScene(Kotonoha::Kotonoha *game, int index) {
  if (index < 0 || index >= (int)game->gameplays.size()) {
    return;
  }

  Kotonoha::Gameplay *play = game->gameplays[index];
  if (play == nullptr) {
    return;
  }
  ...
  editorDirty = false;
  Kotonoha_LoadSceneEditorFromDisk(play);
}

static void Kotonoha_TryAutoLoadCurrentScene(Kotonoha::Kotonoha *game,
...
static void Kotonoha_GoToScene(Kotonoha::Kotonoha *game, Kotonoha_Game &context,
                               int index) {
  if (index < 0 || index >= (int)game->gameplays.size()) {
    return;
  }
  ...
  context.scene = (size_t)index;
  game->gameplays[context.scene]->Reset();
...
static bool Kotonoha_RecreateGameplayAtPath(Kotonoha::Kotonoha *game,
                                            Kotonoha_Game &context, int index,
...
static bool Kotonoha_LoadTemporaryGameplayAt(Kotonoha::Kotonoha *game,
  ...
  // Salva o original apenas na primeira vez
...
static bool Kotonoha_RestoreOriginalPath(Kotonoha::Kotonoha *game,
  ...
  if (temporaryGameplayIndex < 0 ||
      temporaryGameplayIndex >= (int)game->gameplays.size()) {
...
bool Kotonoha_BasicGuiRun(Kotonoha::Kotonoha *game, Kotonoha::Gameplay *play,
  ...
  if (play != nullptr) {
    ...
    ImGui::Text("Current Scene: %d / %d", (int)context.scene + 1,
                (int)game->gameplays.size());
    ImGui::TextWrapped("Current File: %s", play->scriptPath.empty()
                                               ? "<no file path>"
    ...
    if (ImGui::CollapsingHeader("Playback", ImGuiTreeNodeFlags_DefaultOpen)) {
      ...
      if (ImGui::Button("Exit")) {
          Kotonoha_GoToScene(game, context, game->gameplays.size() - 1);
          context.next = true;
      }
    }
    ...
    if (ImGui::CollapsingHeader("Load Scenes",
                                ImGuiTreeNodeFlags_DefaultOpen)) {
      ImGui::InputTextWithHint("##open_script_path",
                               "file1.ks;file2.ks;folder/", openBuf,
  ...
  if (show_playlist) {
    ...
    if (ImGui::CollapsingHeader("Append", ImGuiTreeNodeFlags_DefaultOpen)) {
      ImGui::InputTextWithHint("##append_scene_path", "append file or folder",
                               appendBuf, IM_ARRAYSIZE(appendBuf));
      ImGui::SameLine();
  ...
  if (Kotonoha_BasicGuiEditorShow) {
    ...
    if (selectedSceneIndex >= 0 &&
        selectedSceneIndex < (int)game->gameplays.size()) {
      ...
      if (selected != nullptr) {
        ...
        if (ImGui::Button("Load From Disk")) {
          Kotonoha_LoadSceneEditorFromDisk(selected);
          Kotonoha_RecreateGameplayAtPath(game, context, editorSceneIndex, selected->scriptPath.c_str());
        }
        ...
        if (ImGui::InputTextMultiline("##scene_editor", &sceneEditorText,
                                      avail2)) {
          editorDirty = true;
        }
        ...
        if (ImGui::Button("Save")) {
          if (Kotonoha_SaveAndReloadReplace(game, context,
                                            selectedSceneIndex)) {
            ImGui::End();
            ImGui::Render();
        ...
        if (ImGui::Button("Save Temporary")) {
          if (Kotonoha_LoadTemporaryGameplayAt(game, context,
                                               selectedSceneIndex)) {
            ImGui::End();
            ImGui::Render();
            SDL_SetRenderScale(context.render, io.DisplayFramebufferScale.x,
