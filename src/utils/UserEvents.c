...
void Kotonoha_eventPush(struct Kotonoha_eventStack *stack, SDL_Event event) {
  ...
  struct Kotonoha_eventQueue *newEvent =
      (struct Kotonoha_eventQueue *)SDL_malloc(
          sizeof(struct Kotonoha_eventQueue));
  if (newEvent == NULL) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                 "Falha na alocação de memória para evento");
    return;
  }
...
SDL_Event Kotonoha_eventRead(struct Kotonoha_eventStack *stack,
                             void **persistent) {
  SDL_Event empty = {0};
  ...
  struct Kotonoha_eventQueue *currentEvent =
      (struct Kotonoha_eventQueue *)*persistent;
  ...
  *persistent =
      currentEvent
          ->next; // Atualiza o ponteiro persistente para o próximo evento
  SDL_UnlockMutex(stack->mutex);
